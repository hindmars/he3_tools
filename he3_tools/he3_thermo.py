...
def entropy_density_norm(t, p, phase, squeeze_me=True, diagonal=False):
    ...
    C_V_norm_Tc = h3hc.C_V(1, p, phase)
    
    sf_int_fac = (t**3 - 1)/3
    sf_int_fac[sf_int_fac > 0] = 0.0
    # Entropy density at Tc is equal to the volumetric specific heat, as it is linear in t
    if diagonal:
        s_norm = (h3hc.C_V_normal(t, p, squeeze_me=False, diagonal=True) + sf_int_fac * C_V_norm_Tc)/(c_scale)
    else:
        s_norm = (h3hc.C_V_normal(t, p, squeeze_me=False, diagonal=False)[:,None] + np.outer(sf_int_fac,  C_V_norm_Tc))/(c_scale)
    
    
    if squeeze_me:
        s_norm = h3p.squeeze_float(s_norm)

    # print(t.shape, p.shape, diagonal, s_norm.shape)
    return s_norm
...
def latent_heat_norm(p, squeeze_me=True, diagonal=False):
...
def alpha_pt_AB_norm(t, p, squeeze_me=True, diagonal=False):

    delta_theta = theta_norm(t, p, 'A', squeeze_me, diagonal) - theta_norm(t, p, 'B', squeeze_me, diagonal)
    
    thermal_energy_density = 0.75 * enthalpy_density_norm(t, p, 'A', squeeze_me, diagonal)
