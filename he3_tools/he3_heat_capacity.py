...
# This function now replaced by h3d.ßT_poly_Greywall
#
# def gamma(p):
#     """
...
def heat_capacity_vlow(V, T):
    """
    Eq. (7): Valid below ~7 mK. Uses Greywall 1986 data.
    Inputs:
        V (float): molar volume (cm^3/mol)
        T (float): temperature (K, Greywall scale)
    Returns:
        C_R (float): specific heat in the same units used by the fit (per mole)
    """
    # p = h3p.p_bar_from_molar_vol_cm3(V)
    # C_R = gamma(p) * (h3c.R * T)
    # print(p, gamma(p))
    V_data = h3d.data_Gre86_heat_cap_df['$V\ (\mathrm{cm}^3)$']
    C_RT_data = h3d.data_Gre86_heat_cap_df['$\gamma\ (\mathrm{K}^{-1})$']
    # numpy.interp requires monotonically increasing data
    C_R = T * np.interp(V, V_data[::-1], C_RT_data[::-1])
    return C_R
...
def heat_capacity_low(V, T):
    ...
    Inputs:
        V (float): molar volume (cm^3/mol)
        T (float): temperature (K, Greyeall)
    Returns:
        C_R (float): specific heat in the same units used by the fit (per mole)
    """
...
def heat_capacity_high(V, T):
    ...
    Inputs:
        V (float): molar volume (cm^3/mol)
        T (float): temperature (K, Greywall)
    Returns:
        C_R (float): specific heat in the same units used by the fit (per mole)
    """
    # if T <= 0:
...
def heat_capacity_Greywall(V, T):
    ...
    Returns:
        C_R (float): specific heat (per mole) from the empirical fit.
    """
    T = np.atleast_1d(T)
    low_T = T < 0.1
    C_R = np.zeros_like(T)
    C_R[low_T] = heat_capacity_low(V, T[low_T])  
...
def C_V_normal(t, p, squeeze_me=True, diagonal=False):
    ...
    Uses formula (2.13) from Vollhardt & Woelfle.
    $$
    C_V = \frac{\pi^2}{3} N_F k_B^2 T
    $$
    where $N_F$ is the total density of states, including the spin sum.

    As the density of states is calculated from the heat capacity, this is 
    just a consistency check, to be compared against heat_capacity_normal_liquid.
...
def C_V(t, p, phase, squeeze_me=True, diagonal=False):
    """
    Volumetric heat capacity in given superfluid phase, including normal phase, 
    with strong coupling corrections included.  
    Uses formula (3.77) from Vollhardt & Woelfle.
    $$
    C_V = C_N + \Delta C_V
    $$
